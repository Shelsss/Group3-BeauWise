...
export default function CameraScreen() {
	...
	const focusCamera = useDebouncedCallback(async (x, y) => {
		try {
			...
			if (error.code === 'capture/focus-canceled') return;
		}
	}, 400);

	const tapToFocus = Gesture.Tap().onEnd(({ x, y }) => {
		...
		ringChildScale.value = withTiming(1.14, { duration: 450 });
		ringScale.value = withTiming(1, { duration: 350 });

		ringOpacity.value = withDelay(700, withTiming(0, { duration: 500 }));
		ringChildOpacity.value = withDelay(200, withTiming(0, { duration: 400 }));
	});
