...
export default function AnalysisHistory() {
	...
	const indexOfBatch = historyTabs.indexOf(
		historyTabs.find(({ key }) => key === 'analysis_history')
	);
	...
	const { data, isPending, isError, refetch, isRefetchError } = useQuery({
		queryKey: ['analysis_history', `analysis_history_${filter}`],
		queryFn: queryFn('analysis_history'),
		enabled
	});
	...
	const formatToSection = () => {
		...
		const sectionFormat = sections.reduce((acc, cur) => {
			...
			data?.forEach((item) => {
				const itemDate = parse(item.analysis_check_date, 'p', new Date());
